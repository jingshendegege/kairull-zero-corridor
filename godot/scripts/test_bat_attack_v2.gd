extends SceneTree
## 棍击三连 + 洋葱片虚影无头单测。
## 原测试文件 ACL 受保护；此副本保留其全部覆盖，只升级连续前送的两条预期。
## 跑法：godot --headless --path godot --script scripts/test_bat_attack_v2.gd
...
func _init() -> void:
	...
	# 无方向输入只消费 38px 前送；持续同向移动由 attack_flow 专项覆盖。
	var step_dx: float = player.position.x - x0
	ok(step_dx > 0.0 and step_dx < KairullPlayer.BAT1_STEP * 0.6,
		"首击首帧开始前送但不瞬移到终点", str(step_dx))
	player.keys = {MOUSE_BUTTON_LEFT: true}   # 按住不重复触发
	var x1: float = player.position.x
	for i in range(5):
		player.step(1.0 / 60)
	ok(player.state == "bat1", "按住左键不重复起手", player.state)
	ok(player.position.x > x1 and approx2(player.position.x - x0, KairullPlayer.BAT1_STEP),
		"前摇逐帧走完首击前送，不按方向键时不额外滑行", str(player.position.x - x0))
	...
	# 测试显式释放未挂入树的关卡和玩家，避免退出时误报 CanvasItem 泄漏。
	player.free()
	level.free()
