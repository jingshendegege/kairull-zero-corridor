extends SceneTree
## 四座长程货梯在真正新关中的低站上车→上升→上廊→返回→下落→下车。
...
func _run() -> void:
	for lift_index in 4:
		...
		check(game.moving_lifts.size() == 4 and game.player.moving_platforms.size() == 4,
			"第%d货梯场景接线：四平台实际共享给Player" % (lift_index + 1))
		...
		check(saw_up and top_reached and synced and not lift.stalled,
			"第%d货梯携带真实主角上升576px，不穿过备用中继台/挤头" % (lift_index + 1), str(player.position))
		check(is_equal_approx(lift.bottom_y - lift.top_y, 576.0)
			and absf(player.position.y - (lift.top_y - 0.1)) < 1.1, "上站到达576px连廊实际高度")
		var right_x: float = lift.position.x + 96.0
		check(walk_to(right_x, lift.top_y - 0.1), "从上站不用跳跃直接向右接入主要楼层停靠站")
	...
	print("M05_LIFT_ROUTES_RESULT: %d PASS / %d FAIL" % [passed, failed])
	quit(int(failed > 0))
